...
def test_task2():
    ...
    # Test to see if result[1] = [1, 2, 3, 4]
    b = result[1]
    assert b.shape == (3,), "task2 second result is the wrong shape"

    # Test to see if result[2] = [[a, b, c]
    #                             [d, e, f]]

    c = result[2]
    assert c.shape == (2, 3), "task2 third result is wrong shape"
...
def test_task5():
    ...
    # expected_phi = 3*G
    expected_phi = 3 * G
    assert np.allclose(phi, expected_F), "Potential value not as expected"
