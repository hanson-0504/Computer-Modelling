...
class Particle3D(object):
    ...
    def kinetic_energy(self):
        ...
        ke: float
            1/2 m v**2
        """
        v = math.sqrt(np.dot(self.velocity, self.velocity))
    ...
    def update_position_2nd(self, dt, force):
        ...
        -------
        self.position: numpy array
            r(t+dt) = r(t) + dt v(t) + dt^2 f(t)/m
        """
        self.position = self.position + dt*self.velocity + (dt**2) * (force/self.mass)
    ...
    def total_kinetic_energy(particles):
        ...
        -------
        total_ke: float
            total_ke = ke[0] + ke[1] + ke[2] +...+ ke[n-1]
        """
        total_ke = 0
