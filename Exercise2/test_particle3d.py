...
def test_init():
    ...
    assert np.allclose(p.velocity, [1.0, -1.0, 0.0]), "Particle velocity not set correctly"
...
def test_read_line():
    ...
    assert np.allclose(p_list[0].velocity, [0, 0, 1.0]), "First particle velocity not correct"
    assert np.allclose(p_list[1].velocity, [0.5, 0.5, 0]), "Second particle velocity not correct"
    assert np.allclose(p_list[2].velocity, [-1.0, 1.0, 0]), "Third particle velocity not correct"
